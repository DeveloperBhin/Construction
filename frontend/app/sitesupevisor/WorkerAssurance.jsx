...
const Budget = () => {
  ...
  const fetchBudget = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://192.168.1.150:8000/QualityAssuranceStatus/', {
        method: 'GET',
        headers: {
  ...
  const renderBudgetItem = ({ item, index }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.itemText}>💲 Worker Name: {item.name}
        <TextInput
      ...
      <Text style={styles.itemText}>💲 Worker Email: {item.email}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
              value={item.email} 
              editable={false} 
      ...
      <Text style={styles.itemText}>💲 Worker Contact: {item.phone}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
              value={item.phone}
              editable={false} 
      ...
      <Text style={styles.itemText}>💲 Quality Assurance Status: {item.QualityStatus}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
              value={item.Status} 
              editable={false} 

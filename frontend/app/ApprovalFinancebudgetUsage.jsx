...
const Budget = () => {
  ...
  const handleQualityAssurance = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.150:8000/FinanceBudgetApproval/', {
        method: 'POST',
        headers: {
          "Authorization": "Token 0aacb12174c69ed99e1ab48c305a1000c3f4d482", "Content-Type": "application/json"
        },
        body: JSON.stringify({ Approveform: budget })
      });
  ...
  const renderBudgetItem = ({ item, index }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.itemText}>💲 Budget Name: {item.BudgetName}
        <TextInput
      ...
      <Text style={styles.itemText}>💲 Total Budget: {item.Totalbudget}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
      ...
      <Text style={styles.itemText}>💲 Amount Spent: {item.AmountSpent}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
      ...
      <Text style={styles.itemText}>💲 Remaining Budget: {item.remainingbudget}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
      ...
      <Text style={styles.itemText}>💲 comments: {item.comments}
        <TextInput
              style={{ height: 0, opacity: 0 }} 
              value={item.comments} 
              editable={false} 
      ...
      >
        ...
        <Picker.Item label='Pennding' value='Pennding' />

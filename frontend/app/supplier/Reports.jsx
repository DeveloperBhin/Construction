...
const Report = () => {
  ...
  const handleSubmit = async () => {
    if (materials.some(item => !item.Quantity_Needed || !item.price_Per_Quantity || !item.Total_Amount)) {
  ...
  const renderItem = ({ item, index }) => (
    ...
    <TextInput
      ...
      onChangeText={(text) =>{ setTotal_Amount(text);  updateRow(index, 'Total_Amount', text);}}
    /> </View>
  );
